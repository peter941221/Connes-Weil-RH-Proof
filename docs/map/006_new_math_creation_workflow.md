...
### Formal brick G8 (record 1242, 2026-09-09)

The owner-level algebraic step is now formal. Dev leaf
...
### Generation round BONE-A (record 1416, 2026-09-14)

First candidate generated by the [`010`](010_bone_foundry.md) foundry
procedure - generation from the obstruction ledger rather than from
retrieval, beat 1 being exhausted for this face (see `010` section 0).
Generation pair: operator 6 (negate one scoped hypothesis of a
family-specific no-go) x O2 (radius gap).

```text
+-------+---------------------------------+--------------------------------------+----------------+
| id    | category                        | claimed mechanism                    | status         |
+-------+---------------------------------+--------------------------------------+----------------+
| BONE-A| vanishing-set flexibility /     | a scale-covariant tower producer    | SCREENED-DEAD  |
|       | scale-covariant producer        | vanishing on {0, lam/2, lam} would | (paper stage,  |
|       | (in-project, generated)         | give gate_{F_lam} => SourceRH and   | zero Lean on   |
|       |                                 | intersect the fixed-window          | the attack)    |
|       |                                 | certificates, crossing O2           |                |
+-------+---------------------------------+--------------------------------------+----------------+
| BONE-A'| minimal normal form of the     | reassembly, no mechanism claimed:   | COMMITTED      |
|       | wall (by-product of the screen) | the gate family is monotone in the  | (formal, 13    |
|       |                                 | node set, so every sub-triple F is   | decls, standard|
|       |                                 | SourceRH-equivalent and F = empty is | axioms, 0      |
|       |                                 | the side-condition-free normal form  | sorryAx)       |
+-------+---------------------------------+--------------------------------------+----------------+
```

Kill-ledger row (rule 4: the death is recorded in the same commit):

```text
CANDIDATE   BONE-A scale-covariant producer
VERDICT     SCREENED-DEAD
EVIDENCE    [FORMAL-FAMILY-SPECIFIC] poleTerm_convolutionSquare_of_vanishesOn_
              cc20Triple (C1HealthyYoshidaDetector.lean:102-110) reads ONLY
              CriticalVanishingPoint.half; the pole pair +-1/2 belongs to xi,
              not to the test, so no transform of g relocates it
            [PAPER, law 65] dilation rigidity
              laplaceAt (D_lam g) s = (1/lam) * laplaceAt g (s/lam) forces
              lam = 1 for any node set containing half
ANTI-CIRC   the kill uses only the pole's location and the reverse leg's
              signature; it assumes nothing that is itself open
COST        ~40 min paper + grep, zero Lean spent on the attack
BY-PRODUCT  BONE-A' committed as ConnesWeilRH/Dev/C1MinimalWeilCriterion.lean
              (+ Audit); law F20 born (grep for over-specialized DEFINITIONS,
              not only for existing theorems); 004 section 8 O1/O2 upgraded
              from prose to typed form
FULL RECORD docs/map/010_bone_foundry.md
```

No numerical work is authorized by this row, and BONE-A' carries no sign
claim: it sharpens the open problem rather than advancing it. RH is not
claimed.
