# 033 — R3 physical cutoff factor strong limit
...
For each detector `g`, the cutoff radius is its existing support-owned radius
plus `n + 1`. The finite factor from
`fullBoundaryPositiveOperator` is formally identified as the projection onto
the reflected physical output interval followed by the fixed global
convolution `F_g`. The projection tails vanish in L2, so the factor and its
adjoint converge pointwise. Applying the continuous source inclusion adjoint
gives the same strong convergence for the source compression and its
adjoint. The declarations and acceptance evidence are recorded in
