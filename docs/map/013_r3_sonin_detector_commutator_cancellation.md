...
**Date:** 2026-09-14
**Status:** supporting route candidate, paper-only, not screened live.
...
### SC1 — half-line commutator model

Derive the kernel of the model commutator for a smooth compact convolution.
The output must be a theorem-shaped Schatten-1 condition, not a finite-grid
observation. The model should distinguish:

- raw half-line commutator;
- commutator of the positive square `C†C`;
- the signed outer-pair subtraction.

If the exact detector regularity still gives a non-summable diagonal or
boundary tail, SC1 is killed. That is a useful typed no-go: the proposed
commutator mechanism cannot carry R3 without an additional Sonin cancellation.

### SC2 — Sonin lift

Transfer the model estimate to the actual source Sonin carrier. The lift must
...
**Current verdict:** `PROMISING-BUT-UNSCREENED`. The next allowed action is
the paper-only SC1 half-line kernel falsifier. No Lean brick is authorized by
this record before SC1 survives and a precise source-level target is written.
