...
| `ccm25-finite-prime-index-normalization-discharge.md`; `ccm25-finite-prime-normalization-spine-discharge.md`; `ccm25-finite-prime-normalization-theorem-contract.md`; `restricted-to-full-qw-bridge-theorem-contract.md` | finite-prime and restricted-to-full fields in `WeilFormSymbols` | source prime-power index, visibility in `F_g`, global support, restricted lambda cut, fixed-S visible-prime side condition, von Mangoldt weight, `T(n)` pairing, pointwise term normalization before summation, finite-prime sign ownership, and eventual scalar restriction equality | consistent with global `Psi`, restricted `QW_lambda`, and fixed-S visible-prime side condition |
