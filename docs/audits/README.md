...
| `docs/proofs/ccm25-concrete-normalization-spine.md` | first Lean-facing CCM25 normalization spine; splits the current CCM25 interface into row-level theorem names and exposes the remaining exact-support gap, now guarded by fixed-`lambda` exact-support, source prime-power support, and pointwise finite-prime term modules |
