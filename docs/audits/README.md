...
| `docs/proofs/ccm25-concrete-normalization-spine.md` | first Lean-facing CCM25 normalization spine; splits the current CCM25 interface into row-level theorem names and exposes the remaining exact-support gap, now guarded by fixed-`lambda` exact-support, source prime-power support, pointwise finite-prime terms, and the combined fixed-lambda certificate |
...
| `ccm25-concrete-normalization-axiom-audit.md` | Lean audit for the CCM25 concrete-normalization split; records WSL build results, axiom output, and the fixed-lambda support-plus-term certificate while keeping accepted-source certification open |
...
| `01_TOP_DOWN_TRACE_SCALE_SOURCE_TERM_LEDGER.md` | first S2-B1 accepted-source packet; maps possible positive-trace source terms to `QW_lambda`, rank, pole, or endpoint-strip `Cdef` and names the untracked-bulk obstruction |
