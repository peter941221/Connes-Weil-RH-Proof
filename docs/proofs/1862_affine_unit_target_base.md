...
finite-node base used by the healthy orbit route as an affine right-inverse
correction with target value `1` at every selected node. It exports support,
all finite Laplace target equations, and the uniform quadratic vertical-tail
bound in one owner.
