...
/-!
# Wall-A coefficient bound: log(4*pi) + gamma > 5/2

A sharp-enough lower bound on the archimedean coefficient
``C = log(4.pi) + gamma`` to close the Wall-A 1.4 ``hI`` gate.
The single truly analytic input is ``exp 1 < 11/4``, obtained with the
order-7 Taylor remainder in mathlib's ``Real.exp_bound'``; the rest is
monotone exponent/log algebra: ``exp(12/5) < 12 < 4*pi`` gives
``12/5 < log(4*pi)`` and with ``gamma > 1/2`` yields ``C > 29/10``,
comfortably above the coarse-integral threshold.  RH NOT claimed.
-/

namespace ConnesWeilRH
namespace Source
namespace Dev
namespace Wall14Coeff
...
/-- ``exp 1/2 < 7/4``, the near-band-constant input for Wall-A 1.4 hI. -/
lemma expHalf_lt : Real.exp (1/2 : ℝ) < (7/4 : ℝ) := by
  have h := Real.exp_bound' (x := (1/2 : ℝ)) (by norm_num) (by norm_num) (n := 2) (hn := by norm_num)
  have hle : (∑ m ∈ Finset.range 2, (1/2 : ℝ)^m / m.factorial) +
      (1/2 : ℝ)^2 * (2+1) / (Nat.factorial 2 * (2 : ℕ)) < (7/4 : ℝ) := by
    norm_num [Finset.sum_range_succ]
  nlinarith
