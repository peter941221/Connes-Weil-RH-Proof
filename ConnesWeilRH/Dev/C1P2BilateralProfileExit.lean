...
theorem P2BilateralProfileAggregateWitness.of_bombieriQuadraticResidualP2BridgeData
    (g : CompactLogTest)
    (hvanishes : CC20VanishesOn C1.healthyCC20TestSpace
      cc20TripleFiniteVanishingSet g)
...
/-- The canonical-cutoff Bombieri contract feeds the same aggregate P2 owner.
-/
theorem P2BilateralProfileAggregateWitness.of_bombieriQuadraticCanonicalSpectralTailP2BridgeData
    (g : CompactLogTest)
    (hvanishes : CC20VanishesOn C1.healthyCC20TestSpace
      cc20TripleFiniteVanishingSet g)
...
/-- The Bombieri-native qIntegrand prefix contract feeds the same aggregate
socket through its formal KstarGram/Hermitian adapter. -/
theorem P2BilateralProfileAggregateWitness.of_bombieriQuadraticCanonicalQIntegrandPrefixP2BridgeData
    (g : CompactLogTest)
    (hvanishes : CC20VanishesOn C1.healthyCC20TestSpace
      cc20TripleFiniteVanishingSet g)
  ...
  hbalance :=
    (qw_nonneg_iff_archimedean_plus_bilateralProfile_weighted_sum_nonpos
      g hvanishes).mp
      (qw_nonneg_of_bombieriQuadraticCanonicalPrefixP2BridgeData
        p.toCanonicalPrefix)
...
contract.  The finite visible-prime owner remains explicit in the producer
quantifier, while the residual budget is consumed on the same `g`. -/
theorem sourceRH_of_pinnedOrbitDetector_p2BombieriQuadraticResidualP2BridgeData
    (hproducer : ∀ rho : sourceNontrivialZeroSet,
      (1 / 2 : Real) < rho.1.re →
        ∃ g : CompactLogTest, ∃ n : Nat,
          HealthyYoshidaDetectorData rho.1 g ∧
          ...
          (∀ q ∈ globalPrimeIndexSet g.convolutionSquare,
            (q : Real) < Real.exp (2 * ((n + 2 : Nat) : Real))) ∧
          Nonempty (BombieriQuadraticResidualP2BridgeData g)) :
    RHDefinitionBridge.standard.SourceRH := by
  apply healthy_sourceRH_of_right_detector_specific_qw_nonneg
  intro rho hright
  obtain ⟨g, _n, hdata, _hsupport, _hvisible, ⟨hp2⟩⟩ :=
...
/-- Pinned same-detector exit for the canonical-cutoff Bombieri contract. -/
theorem sourceRH_of_pinnedOrbitDetector_p2BombieriQuadraticCanonicalSpectralTailP2BridgeData
    (hproducer : ∀ rho : sourceNontrivialZeroSet,
      (1 / 2 : Real) < rho.1.re →
        ∃ g : CompactLogTest, ∃ n : Nat,
          HealthyYoshidaDetectorData rho.1 g ∧
          ...
          (∀ q ∈ globalPrimeIndexSet g.convolutionSquare,
            (q : Real) < Real.exp (2 * ((n + 2 : Nat) : Real))) ∧
          Nonempty
            (BombieriQuadraticCanonicalSpectralTailP2BridgeData g)) :
    RHDefinitionBridge.standard.SourceRH := by
  apply healthy_sourceRH_of_right_detector_specific_qw_nonneg
  intro rho hright
  obtain ⟨g, _n, hdata, _hsupport, _hvisible, ⟨hp2⟩⟩ :=
...
/-- Pinned same-detector exit for the Bombieri-native qIntegrand prefix
contract. -/
theorem sourceRH_of_pinnedOrbitDetector_p2BombieriQuadraticCanonicalQIntegrandPrefixP2BridgeData
    (hproducer : ∀ rho : sourceNontrivialZeroSet,
      (1 / 2 : Real) < rho.1.re →
        ∃ g : CompactLogTest, ∃ n : Nat,
          HealthyYoshidaDetectorData rho.1 g ∧
          ...
          (∀ q ∈ globalPrimeIndexSet g.convolutionSquare,
            (q : Real) < Real.exp (2 * ((n + 2 : Nat) : Real))) ∧
          Nonempty
            (BombieriQuadraticCanonicalQIntegrandPrefixP2BridgeData g)) :
    RHDefinitionBridge.standard.SourceRH := by
  apply healthy_sourceRH_of_right_detector_specific_qw_nonneg
  intro rho hright
  obtain ⟨g, _n, hdata, _hsupport, _hvisible, ⟨hp2⟩⟩ :=
  ...
  exact ⟨g, hdata,
    qw_nonneg_of_healthyDetectorData_of_bombieriQuadraticCanonicalSpectralTailP2BridgeData
      hdata hp2.toCanonicalPrefix.toCanonicalSpectralTail⟩
