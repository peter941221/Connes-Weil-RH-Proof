...
structure CCM24SourceModel where
  ...
  windowContainedInLambda :
    ∀ (I : semilocalSymbols.Window) (lambda : ℝ),
      1 < lambda → semilocalSymbols.windowContainedInLambda I lambda
...
theorem ccm24_source_window_contained_in_lambda
    (M : CCM24SourceModel) (I : M.semilocalSymbols.Window)
    {lambda : ℝ} (hlambda : 1 < lambda) :
    M.semilocalSymbols.windowContainedInLambda I lambda :=
  M.windowContainedInLambda I lambda hlambda

theorem ccm24_source_lambda_compatible
    (M : CCM24SourceModel) (I : M.semilocalSymbols.Window)
    {lambda : ℝ} (hlambda : 1 < lambda) :
    M.semilocalSymbols.lambdaCompatible I lambda :=
  SemilocalModelSymbols.lambdaCompatible_of_windowContainedInLambda
    (M.windowContainedInLambda I lambda hlambda)
...
structure CCM24SemilocalObjectPackage where
  ...
  sourceFourierSupportInvolutionGeometryData :
    SemilocalModelSymbols.FourierSupportInvolutionGeometryData
      sourceModel.semilocalSymbols sourceSupportWindow
  ...
  sourceBackedBoundedComparisonMap :
    sourceBackedBoundedComparisonData →
      sourceModel.semilocalSymbols.boundedComparisonMap
        sourceModel.semilocalSymbols.sourcePlaceSet
  sourceBackedBoundedComparisonInverse :
    sourceBackedBoundedComparisonData →
      sourceModel.semilocalSymbols.boundedComparisonInverse
        sourceModel.semilocalSymbols.sourcePlaceSet

namespace CCM24SemilocalObjectPackage

def semilocalSymbols (ccm24 : CCM24SemilocalObjectPackage) :
    SemilocalModelSymbols :=
  ccm24.sourceModel.semilocalSymbols

end CCM24SemilocalObjectPackage
