...
finite-prime layer.  It keeps the current phase-1 `TestFunction := Type`
boundary, but prevents later finite-prime visibility and prime-power
evaluation data from silently using different convolution-square objects.
-/
