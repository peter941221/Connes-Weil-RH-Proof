...
structure FixedSTestObligationData
  ...
  semilocalTest_eq_sourceLeg :
    pkg.ccm24.semilocalSymbols.sourceTest = pkg.ccm24.semilocalSymbols.sourceTest := rfl
...
def toFixedSTestFrontEndData
  ...
  finitePrimeVisibilityStatement := by
    ...
    exact data.finitePrimeVisibilityStatement
...
theorem source_backed_semilocal_test_eq_source_leg
    ...
    (SourceBackedFixedSTest.ofExpandedSourcePackage pkg
      ...
      pkg.ccm24.semilocalSymbols.sourceTest :=
  rfl
...
the source-test arithmetic certificate owner attached to the package common
test.
-/
theorem finite_prime_visibility_statement_of_concrete_arithmetic_rows
    ...
    WeilFormSymbols.FinitePrimeVisibilityStatement
      pkg.ccm25.weilSymbols pkg.commonTest.sourceTest
      pkg.commonTest.sourceTest :=
  (Source.CCM25Concrete.FinitePrimeInterface.finite_prime_normalization_of_common_source_test_certificates
    (Source.CCM25Concrete.FinitePrimeSourceData.fixedLambdaArithmeticSourceTestCertificatesForAllTests
      pkg.commonFinitePrimeArithmeticSourceDataOwner))
    pkg.commonTest.sourceTest pkg.commonTest.sourceTest

theorem finite_primes_visible_of_obligation_data
    (pkg : Source.SourceObject.SourceObjectPackage)
    (data : FixedSTestObligationData pkg) :
    data.test.finitePrimesVisible :=
  data.finitePrimeVisibilityBridge
    data.finitePrimeVisibilityStatement
...
theorem admissible_for_theorem1_of_obligation_data
  ...
  ⟨data.admissibleWindow,
    finite_primes_visible_of_obligation_data pkg data⟩
