...
- **The real-test phase leaf exposes an interface, not the missing bound** (Observed 2026-08-25, `C1SpectralRealPair`): `spectralTerm_convolutionSquare_re_of_isReal` is an exact reduction to `Re(L_g(-w) * L_g(w))`, `rightHalfPhaseKernel` names that multiplicity-weighted phase term, and `rightHalfSpectralSum_re_eq_tsum_indicator_phase` is only the `Complex.re_tsum` total-series readback. `rightHalfPhaseBound_onRealVanishing` names the restricted proposition but does not prove it; its `_iff_spectral` companion is only a presentation equivalence. `norm_convolutionSquare_test_zero_eq_integral_normSq` only reads the origin mass. Do not cite any of these as an off-origin autocorrelation estimate or as the W4b inequality; the phase bound remains an independent analytic premise. The remaining obligation is now packaged: `rightHalfPhaseBound_of_prefix_tail_certificate` proves that one finite zero prefix whose phase sum exceeds the target by epsilon with audited tail norm below epsilon suffices, and `rightHalfPhasePrefixCertificate_onRealVanishing` names the per-test proposition (such a prefix for every positive margin); `rightHalfPhaseBound_onRealVanishing_of_prefix_certificate` assembles it. Producing the certificate is still the open analytic part of W4b.  Build #24 refined the quantifier to one EXISTENTIAL margin per test (`not_rightHalfPhasePrefix_unboundedMargins` refutes the for-all-margin form via absolute summability) and proved `rightHalfPhaseStrictBound_iff_prefixCertificate`: per real test, that certificate is exactly the STRICT bound (total phase tsum > -(1/2)*onLineSpectralMass), with the non-strict target following by assembly.  Two pitfalls observed while building it: in a goal shaped like `a > b ↔ ...` remember that `a > b` parses as `b < a`, so `constructor` direction order follows that parsing (swap the iff sides or the bullets); and `rw` does NOT unfold local `set`-bound names - rewrite them explicitly through their equality witnesses first (e.g. `rw [hSphase, hsplit]`).
...
- **Term-mode `HasDerivAt.comp` defers its inline arguments to metavariables — use the named shift lemmas instead** (Observed 2026-08-26, `C1SpectralW4bBoundary.cubicLogTower_gap_hasDerivAt`): with no expected type, `HasDerivAt.comp A B` elaborates `A` against a slot whose point is an unresolved meta application, so the whole application fails "expected to have type HasDerivAt ?m ?m (?m ?m)" even when both arguments are fully concrete; `convert!` alone does NOT fix it because the term must elaborate standalone. The landed fix: `HasDerivAt.comp_const_add (a : 𝕜) x hf` (`Mathlib/Analysis/Calculus/Deriv/Shift.lean`) states exactly `HasDerivAt (fun y ↦ f (a + y)) f' x` from `hf : HasDerivAt f f' (a + x)` — for `log (1 + t)` take `a := 1` with `hf = Real.hasDerivAt_log (ne_of_gt hpos)`, and the conclusion lambda AND value are already in clean form, so no normalization bridge is needed. Fallbacks: split arguments into named local constants, or `HasDerivAt.comp_of_eq`.

- **Untyped numeric literals float the field to ℕ in implicit-point deriv lemmas** (Observed 2026-08-26, same leaf): `hasDerivAt_id 0`, `HasDerivAt.div_const _ 2` etc. fail "failed to synthesize instance NontriviallyNormedField ℕ" because the literal's field is inferred from an unresolved typeclass float when the point argument leaves it ambiguous. Type every numeric argument at the target field: `(0 : ℝ)`, `(2 : ℝ)`.

- **v4 `DifferentiableOn g s` unfolds to `DifferentiableWithinAt` bullets, and interval membership iffs are `Set.`-prefixed** (Observed 2026-08-26, same leaf): after `intro x hx`, each bullet's goal is `DifferentiableWithinAt g s x`; close the boundary bullet with `h.differentiableWithinAt` from a `DifferentiableAt`, and interior bullets via `(hasDeriv).differentiableAt.differentiableWithinAt`. The iff lemmas are `Set.mem_Ici` / `Set.mem_Ioi` — bare `mem_Ici`/`mem_Ioi` do not resolve in a leaf without an explicit open (mathlib's own bare uses live where `Set` is opened).

- **Plain `def`s over ℝ division need `noncomputable`, and the check only fires on fully green builds** (Observed 2026-08-26, same leaf): `dependsOnNoncomputable` runs at CODEGEN — while type errors elsewhere keep the module red you never see it, so a def like `def cubicLogTower (t : ℝ) := t - t^2/2 + ...` compiles "fine" in every red build and explodes once everything else is green. Mark such defs noncomputable preemptively; leaf convention: all ℝ-valued analytic defs are `noncomputable def`.

- **Mirror probes must import project leaves, not Mathlib umbrellas — and v4.30 `example` takes no name** (Observed 2026-08-26, WSL probe rounds for the second boundary rung): the ext4 mirror only builds the project's transitive closure, so a standalone probe file importing umbrella modules (`Mathlib.Tactic`, …) fails on unbuilt dependencies; import the specific project leaf under test instead and run with `~/.elan/bin/lake env lean ProbeX.lean`. Separately, v4.30 named examples fail "Failed to infer type of binder" (it interprets the name as a parameter) — use theorem/lemma in probe files.
...
- **Warm-mirror cost model (measured 2026-08-25, ext4 mirror, 12 cores):** on a warm `.lake`, a no-op leaf build (3640 jobs) takes ≈3 s and a no-op full-root `lake build ConnesWeilRH` (4147 jobs) ≈3.3 s — job counts are replay bookkeeping, not cost. The dominant per-edit cost is re-elaborating the ONE edited file (≈12 s for a 1224-line Dev leaf). Consequences: (a) a warm root build as a pre-commit gate is cheap insurance even when the edited module is an importer-free leaf — the only thing it can catch is accidental edits to OTHER files; (b) never judge the ladder by job count; (c) a genuinely cold cache (fresh mirror / toolchain bump) is the only scenario where the 3640/4147 closure costs real time — keep the persistent mirror warm instead of "optimizing" iteration builds. Lake v4.30 default output prints NO `Built:`/`Replayed:` lines, so "trust the Built line" is unenforceable without `-v`; the rm-olean trick is the practical proof of re-elaboration.
- **Bare `lake build ConnesWeilRH` does NOT compile standalone top-level `Dev/` leaves:** the default target is the auto-globbed roots plus their transitive import closure — which reaches nested `Source/**` but NOT a top-level `Dev/<leaf>.lean` that nothing in that tree imports. So a green "full root 4147/4147" proves NOTHING about a NEW Dev brick you just wrote: the module was simply never in the target graph, so its errors are invisible and its absence does not fail the build (`lakefile.toml` has no `srcDir`/`modules`/`exclude`, i.e. default auto-glob; there is no `.lakeignore`). Verify each Dev brick by EXPLICIT targeting — `lake build ConnesWeilRH.Dev.<Module>` plus a focused `#print axioms` audit — per the per-brick protocol below, and treat the aggregate only as a batch-level regression gate for the non-Dev Source tree. (Observed 2026-08-26, session 15: C1CC20LpOperator AND prior-session C1YoshidaTranslation were both absent from the 4147-job aggregate yet green under explicit targeting.)
